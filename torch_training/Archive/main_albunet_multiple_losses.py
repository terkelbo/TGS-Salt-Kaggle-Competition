# -*- coding: utf-8 -*-
"""
Created on Sat Oct  6 11:51:28 2018
...
from torch_models.albunet101_SE_hyper_multiple import get_model
...
#training constants
parameter_path = 'CV5_resnet101_weighted_loss_no_drop_low_pixels_two_stage_SE_stratified_on_plateau_adam_hyper_decoder_multiple_loss'
submission_name = 'CV5_resnet101_weighted_loss_no_drop_low_pixels_two_stage_SE_stratified_on_plateau_hyper_decoder_multiple_loss-tta.csv'
...
for j, idx in enumerate(fold.split(file_list,mask_class)):
    ...
    for opt_round in range(2):
        i = 0
        if opt_round == 0:
            seg_loss = lambda pred, target: 5*diceloss(sigmoid(pred),target) + bceloss(sigmoid(pred),target)
            class_loss = lambda pred, target: bceloss(sigmoid(pred),target)
        else:
            patience = 30
        ...
        for e in range(epoch):
            train_loss = []
            model.train(True)
            for image, mask, mask_class, target in tqdm(data.DataLoader(dataset, batch_size = 16, shuffle = True)):        
                image = image.type(torch.FloatTensor).cuda()
                y_pred, image_pool, hyper_non_interpolated = model(image)
                
                mask_class_logic = (np.array(mask_class) == 0)
                
                loss_hyper = 0
                if len(np.argwhere(mask_class_logic).reshape(-1)) > 0:
                    for hypercolumn in hyper_non_interpolated:
                        with torch.no_grad():
                            downsampled = F.interpolate(hypercolumn[np.argwhere(mask_class_logic).reshape(-1)].cuda(),
                                                         size=hypercolumn.shape[-1], 
                                                         mode='bilinear', align_corners=True)
                        loss_hyper += 0.1*seg_loss(hypercolumn[np.argwhere(mask_class_logic).reshape(-1)], downsampled)
                    

                
                #loss_class = 0.05*class_loss(image_pool.view(image.size(0),-1),target.repeat((1,image_pool.size(1))).cuda())                
                
                loss_out = seg_loss(y_pred,mask.cuda())         
                
                loss = loss_hyper + loss_out
