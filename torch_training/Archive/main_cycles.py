...
#training constants
parameter_path = 'CV5_resnet34_weighted_loss_cycles'
submission_name = 'CV5_resnet34_weighted_loss_cycles-tta.csv'
...
cutoff = 10
removed_images = [path.split('/')[-1][:-4] for path, nb_pixels in num_pixels_sorted if int(nb_pixels) < cutoff and int(nb_pixels) != 0]
...
train_idx = list(np.random.choice(list(range(len(file_list))),size=int(0.92*len(file_list))))
val_idx = [idx for idx in list(range(len(file_list))) if idx not in train_idx]

#92/8 % train/val split
