...
class SeNet154(nn.Module):
    ...
    def __init__(self, num_classes=1, num_filters=32, pretrained=False, is_deconv=True):
        ...
        self.resnet_layers = list(pretrainedmodels.__dict__['senet154'](pretrained='imagenet').children())
        self.maxpool = self.resnet_layers[0][3]
        self.conv_in = nn.Conv2d(3, 128, kernel_size=7, stride=1, padding=3, bias=False)
        ...
        self.conv1 = self.resnet_layers[1]
        self.conv2 = self.resnet_layers[2]
        self.conv3 = self.resnet_layers[3]
        self.conv4 = self.resnet_layers[4]
        ...
        self.dec4 = DecoderBlockV2(bottom_channel_nr + 512, 512, 64, is_deconv)
        self.dec3 = DecoderBlockV2(1024 + 64, 256, 64, is_deconv)
        self.dec2 = DecoderBlockV2(512 + 64, 128, 64, is_deconv)
        self.dec1 = ConvRelu(256 + 64, 64)
    ...
    def forward(self, x):
        conv1 = self.conv1(self.conv_in(x)) #; print(conv1.size())
        ...
        center1 = self.center1(conv4) #; print(center1.size())
        center2 = self.center2(center1) #; print(center2.size())
        
        center = self.center_decode(torch.cat([center1,center2],1)) #; print(center.size())

        dec4 = self.SE6(self.dec4(torch.cat([center, conv4], 1))) #; print(dec4.size())
        dec3 = self.SE7(self.dec3(torch.cat([dec4, conv3], 1))) #; print(dec3.size())
        dec2 = self.SE8(self.dec2(torch.cat([dec3, conv2], 1))) #; print(dec2.size())
        dec1 = self.SE9(self.dec1(torch.cat([dec2, conv1], 1))) #; print(dec1.size())
        ...
        cat = torch.cat([
                    dec1,
                    dec2,
                    F.interpolate(dec3, scale_factor = 2, mode='bilinear', align_corners = False),
                    F.interpolate(dec4, scale_factor = 4, mode='bilinear', align_corners = False),
...
def get_model(num_classes=1, num_filters=64, pretrained=False, is_deconv=True):
    model = SeNet154(num_classes, num_filters, pretrained, is_deconv)
...
#model = get_model()
#A = torch.Tensor(1,3,128,128).fill_(1).cuda()
#with torch.no_grad():
